...
u_term = interp1(Prob.user.t_nom,Prob.user.u_nom,t_star);

NMPC_CONJ(n*(N+1)+2,n*N+1:n*(N+1)) = 2*(P*(xu(n*N+1:n*(N+1))-x_term'))';
NMPC_CONJ(n*(N+1)+2,end) = -2*(P*(xu(n*N+1:n*(N+1))-x_term'))'*(f(x_term')+B*u_term');
