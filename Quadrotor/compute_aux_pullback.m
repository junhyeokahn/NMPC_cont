function [uc_fb, E] = compute_aux_pullback(f_ctrl,B_ctrl,lambda, M_xi, xc_nom, yaw_nom, uc_nom, xc, yaw)
...
if (a <= 0) || (norm(b)<= 1e-3) 
    u_metric_fb = zeros(3,1); %[th_dot;rd;pd]
else
    u_metric_fb = -(a/(b'*b))*b; %[th_dot;rd;pd]
end
